  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div>
          <h2 className="text-2xl font-bold text-[#dce1fb] mb-2">
            Staff Management
          </h2>
        ...
        <button
          ...
          className="flex items-center justify-center gap-2 bg-[#7bd0ff] text-[#0c1324] px-4 py-2 rounded-lg font-semibold hover:opacity-90 transition-opacity w-full sm:w-auto"
        >
          <Plus size={18} />
          Add Staff Member
        </button>
      ...
      {/* Staff Table - Desktop */}
      <div className="hidden lg:block bg-[#151b2d] rounded-xl overflow-hidden">
          ...
          <tbody>
            {staff.map((member) => (
              <tr key={member.id} className="border-t border-[#191f31]">
                <td className="px-6 py-4 text-[#dce1fb] font-semibold">
                  {member.name}
                </td>
      ...
      <div className="lg:hidden space-y-4">
        {staff.map((member) => (
          <div key={member.id} className="bg-[#151b2d] rounded-xl p-4">
            <div className="flex items-start justify-between mb-3">
              <div>
                <h3 className="text-[#dce1fb] font-semibold">{member.name}</h3>
                <p className="text-[#dce1fb]/70 text-sm">{member.email}</p>
              </div>
              <span
                className={`px-3 py-1 rounded-full text-xs font-semibold ${
                  member.status === "active"
            ...
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-2">
                <label className="text-[#dce1fb]/70 text-xs uppercase tracking-wide">
                  Role:
                </label>
                <select
                  value={member.role.toLowerCase()}
                  onChange={(e) => handleChangeRole(member.id, e.target.value)}
                  className="bg-[#191f31] text-[#dce1fb] px-2 py-1 rounded text-sm outline-none focus:ring-2 focus:ring-[#7bd0ff]/50"
                >
                  <option value="radiologist">Radiologist</option>
                  <option value="clinician">Clinician</option>
                  <option value="org-admin">Org Admin</option>
      ...
      {showAddPanel && (
        <div className="fixed inset-0 bg-[#0c1324]/80 z-50 flex items-center justify-center lg:justify-end">
          <div className="bg-[#151b2d] w-full max-w-md lg:max-w-md h-full lg:h-full p-4 lg:p-6 overflow-y-auto">
            <div className="flex items-center justify-between mb-6">
              <h3 className="text-xl font-bold text-[#dce1fb]">
                Add Staff Member
              </h3>
              <button
